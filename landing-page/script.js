// Waitlist Form Handler + Decorative Elements (same as home page)
...
function createFlowerGarden() {
    ...
    for (let i = 0; i < flowerCount; i++) {
        const sectionStart = 7.5 + (i * sectionWidth); // Start at 7.5% with proper spacing
        // Smaller offset to prevent overlap - only use 40% of section width
        const offset = Math.random() * (sectionWidth * 0.4);
        const position = sectionStart + offset;

        // Ensure minimum distance from previous flower
        if (i > 0 && position - positions[i - 1] < minSpacing) {
            positions.push(positions[i - 1] + minSpacing);
        } else {
...
function initWaitlistForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            // Use Web3Forms API (free, no signup required)
            const response = await fetch('https://api.web3forms.com/submit', {
                ...
                body: JSON.stringify({
                    access_key: '9389b66f-5f55-42fa-a684-097b935580d0',
                    email: email,
                    subject: 'New Waitlist Signup - Mówka',
                    from_name: 'Mówka Waitlist'
                }),
            });
